...
def iterator(op, K, opt_type, price, S0, R, T, y, orig):
    ...
    for i in range(0, len(S0) - 1):  # Iterate over all options
        ...
        if opt_type[i] == "call":  # Check if call
            sum_c = (
                sum_c + (price[i] - call_price_calc(given, op)) ** 2
            )  # Add to least squared criterian for call options
        else:
            sum_p = (
                sum_p + (price[i] - put_price_calc(given, op)) ** 2
            )  # Add to least squared criterian for put options
    return (
        sum_c + sum_p + a * np.linalg.norm(op - orig)
    )  # Return total least squared + regularization factor * eucleidean distance
...
def optimizer(sigma, lemu, lemd, etau, etad):
    ...
    estimates = optimize.minimize(
        iterator,
        op,
        args=(K, opt_type, price, S0, R, T, y, orig),
        method="L-BFGS-B",
        bounds=bnd,
        options={"disp": True},
    )
